...
const ComparisonView = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        const quotesResponse = await axios.get(`${apiUrl}/projects/getProjectQuotes/${nameProjectId}`);
        quotesLocal = quotesResponse.data;
        setQuotes(quotesLocal); // Update state

        const projectResponse = await axios.get(`${apiUrl}/projects/getOneProject/${nameProjectId}`);
